...
:::
`Webpack`的`Plugin`机制让其更加灵活，以适应各种应用场景。在`Webpack`运行的生命周期中会**广播出许多事件**，`Plugin`可以监听这些事件，在合适的时机通过`Webpack`提供的`API`改变输出结果。
...
`Webpack`的事件流机制应用了观察者模式，和`Node.js`中的`EventEmitter` 非常相似。`Compiler和Compilation`都继承自`Tapable`，可以直接在 `Compiler和Compilation`对象上广播和监听事件，方法如下：
```js
/**
* 广播事件
...
```
## 实战
在项目根目录新建`plugins`目录，用来存放自定义插件；然后新建`my-webpack-plugin`，即我们的第一个自定义插件。具体内容如下：
