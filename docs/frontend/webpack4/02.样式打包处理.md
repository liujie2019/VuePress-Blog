...
[具体参考](https://www.jianshu.com/p/0f55bd17f64f)
```js
// index.less
@import './avatar.less';
```
...
```
importLoaders默认值为0。我们在index.js中引入了index.less，又在index.less中引入了avatar.less(`@import './avatar.less';`)。如果不配置`importLoaders: 2`，那么avatar.less将不会被postcss-loader和less-loader处理。如果css-loader后面只有postcss-loader，则设置`importLoaders: 1`。
