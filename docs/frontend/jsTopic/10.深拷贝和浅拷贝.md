...
:::
1. **深拷贝**会在计算机中**开辟一块新的内存空间**来存放复制的对象，将原对象的各个属性逐个复制出去；
2. 而浅拷贝仅仅是指向被复制对象的内存地址，如果原地址中对象被改变了，那么浅拷贝的对象也会相应改变。
...
```
::: warning
需要注意：扩展运算符实现的也是浅拷贝，无法处理数组元素是引用类型的情况。
:::
```js
const arr = [1, 2, [3, 4]];
const arr2 = [...arr];
...
console.log(arr); // [1, 2, [3, 666]]
```
因此，浅拷贝只能断开一层的引用，如果数据结构是多层引用类型的话，浅拷贝就不能解决问题了，这时候我们需要用到深拷贝。
### 循环拷贝
```js
var arr = [2, 3, 4];
var arr2 = [];
...
const deepClone = obj => {
    ...
    for (let key in obj) {
        ...
        if (obj.hasOwnProperty(key)) {
            // 核心部分，判断是对象类型则递归拷贝
            new_obj[key] = isObject(obj[key]) ? deepClone(obj[key]) : obj[key];
        }
    }
    return new_obj;
};
...
## 参考文档
1. [头条面试官：你知道如何实现高性能版本的深拷贝嘛？](https://mp.weixin.qq.com/s/M7KBX3w2KqlWhZFHJSYP6Q)
...
3. [深拷贝的终极探索（90%的人都不知道）](https://juejin.im/post/5bc1ae9be51d450e8b140b0c#heading-3)
4. [JavaScript专题之深浅拷贝](https://juejin.im/post/59658504f265da6c415f3324)
5. [js中对象的复制，浅复制（浅拷贝）和深复制（深拷贝）](https://www.jianshu.com/p/0d7bd31ccf43)
6. [浅谈js中的浅拷贝和深拷贝](https://www.cnblogs.com/MECN/p/6606119.html)
7. [javascript中的深拷贝和浅拷贝？](https://www.zhihu.com/question/23031215)
8. [React 数据为什么要使用immutable方式？浅复制与深复制思考](https://segmentfault.com/a/1190000006729489)
9. [面试官:请你实现一个深克隆](https://juejin.im/post/5abb55ee6fb9a028e33b7e0a)
10. [js浅拷贝与深拷贝方法](https://segmentfault.com/a/1190000016440069#articleHeader7)