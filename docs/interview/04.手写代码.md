...
```
## 数组相关
### 70个数组元素生成一个二维数组(reduce实现)
reduce用法：

* 如果没有提供initialValue，那么reduce的第一轮回调函数中的prev就是arr[0]，cur就是arr[1]，index就是1；
* 如果提供initialValue，那么reduce的第一轮回调函数中的prev就是initialValue，cur就是arr[0]，index就是0。
```js
